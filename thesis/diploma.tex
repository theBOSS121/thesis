...
\noindent
Zahvaljujem se mentorici izr. prof. dr. Poloni Oblak za pomoč pri izdelavi diplomskega dela.\\
\rm\normalfont
...
Graf $G = (V(G), E(G))$ ima množico vozlišč $V(G)$ in množico povezav $E(G)$. Soseščina $N_G(u)$ vozlišča $u$ iz grafa $G$ so vsa vozlišča $v \in V(G)$, za katera velja $uv \in E(G)$. Soseščina je zaprta, če vsebuje tudi vozlišče $u$, kar označimo z oznako $N_G[u]$. Premer grafa je najdaljša pot med dvema vozliščema. Disjunktna unija grafov je združitev dveh grafov v večji graf tako, da naredimo disjunktno unijo množic vozlišč in disjunktno unijo množic povezav. Komplement grafa $G$ označimo z $\overline{G}$ (nepovezave grafa $G$ so povezave grafa $\overline{G}$). $K_n$ je poln graf na $n$ vozliščih, $\overline{K_n}$ je nepovezan graf na $n$ vozliščih, $P_n$ je pot na $n$ vozliščih ter $K_{m, n}$ je dvodelen graf z $m$ vozlišči v eni in $n$ vozlišči v drugi množici. Drevo je (neusmerjen) povezan graf brez ciklov. Določiti smer povezave $uv$ grafa $G$ pomeni spremeniti povezavo $uv$ v urejen par $(u, v)$ ali $(v, u)$. Usmerjen graf je graf, ki ima vse povezave usmerjene. Orientacija grafa $G$ je usmerjen graf, ki je pridobljen tako, da vsaki povezavi grafa $G$ določimo smer.
...
\begin{itemize}
    ...
    \item linearna urejenost, če je $R$ antisimetrična, strogo sovisna in tranzitivna,
    \item stroga delna urejenost, če je $R$ asimetrična in tranzitivna,
    \item stroga linearna urejenost, če je $R$ asimetrična, sovisna in tranzitivna.
\end{itemize}
...
Kompozitum preslikav, ki ga označimo z $\circ$, je operacija na množici preslikav. Kompozitum preslikav $\pi \circ \sigma$ je taka preslikava, ki najprej element preslika s $\sigma$, nato pa dobljeni element preslika še s $\pi$.
...
Naj bosta $\sigma, \pi \in S_n$. Permutacija $\sigma$ je manjša ali enaka od permutacije $\pi$ v šibki Bruhatovi delni urejenosti, kar označimo s $\sigma \preceq_b \pi$, če je $I_{\sigma} \subseteq I_{\pi}$. 
\end{definicija}
...
\]
Permutacijski graf permutacije $\sigma$ je tak, kot v dokazu trditve \ref{trditev_pot_permutacijski_graf}. Permutacijski graf permutacije $\pi$ je zrcalna slika permutacijskega grafa permutacije $\sigma$.
...
\end{trditev}
\begin{dokaz}
    Vsako drevo z vsaj eno povezavo ima natanko dve tranzitivni orientaciji. To sledi iz dejstva, da ko usmerimo eno povezavo, smo že določili tranzitivno orientacijo drevesa. Zato ima tudi gosenica $C_k(m_1, m_2, \dots, m_k)$ natanko dve tranzitivni orientaciji (slika \ref{tranzitivni_orientaciji_gosenice}).
...
\begin{dokaz}
    ...
    $(\Leftarrow)$ Obratno privzamimo, da so $G, H_1, H_2, \dots, H_n$ permutacijski grafi. Naj bo $(v_1, v_2, \dots, v_n)$ kohezivno zaporedje grafa $G$. Naj bo $n_i$ število vozlišč grafa $H_j$, ki pripada vozlišču $v_i$. Potem ima graf $H_j$, ki pripada vozlišču $v_i$, kohezivno zaporedje $l_i = (x_1^i, x_2^i, \dots, x_{n_i}^i)$. Se pravi je 
    \[
...
Izrek \ref{izrek_neprimaren_graf_je_kompozicija_z_primarnim} nam opiše strukturo permutacijskih grafov. Vsak permutacijski graf je primaren ali neprimaren. Če je neprimaren, je kompozicija permutacijskih grafov s primarnim permutacijskim grafom. Vidimo, da lahko vsak permutacijski graf izrazimo s kompozicijami in primarnimi grafi.
...
V tem poglavju bomo spoznali, kaj so rangiranja, tekmovalnostni grafi, primerljivostni grafi, množice tekmovalcev ter množice posrednih in neposrednih tekmovalcev. Nato si bomo ogledali algoritem za izračun množice posrednih in neposrednih tekmovalcev ter kako lahko množice posrednih in neposrednih tekmovalcev uredimo. Na koncu si bomo pogledali še primer na resničnih podatkih.
...
\begin{definicija}
    Naj bo $R = \{ c_1, c_2, \dots, c_r \}$ množica rangiranj množice $[n]$. Tekmovalnostni graf množice rangiranj $R$ definiramo kot neusmerjen graf $G_c(R) = ([n], E)$, kjer je množica povezav $E$ podana na naslednji način: med $i$ in $j$ je povezava, če $(i, j)$ tekmujeta.
\end{definicija}
...
\label{primer_tekmovalnostni_graf}
    ...
    \end{align*}
    Ker je $c_1=id$, so povezave grafa $G_c(R)$ ravno inverzije rangiranj $c_2$ in $c_3$. Rangiranje $c_2$ ima inverzijo $(4, 3)$, medtem ko ima rangiranje $c_3$ inverziji $(3, 1)$ in $(3, 2)$. Graf $G_c(R)$ je prikazan na sliki \ref{graf_tekmovalnosti_primer}.
    \begin{figure}[ht]
        \begin{center}
            \begin{tikzpicture}[shorten >=1pt,-]
                \tikzstyle{vertex}=[circle,draw=black,fill=white!25,minimum size=20pt,inner sep=0pt]
...
\begin{definicija}
    Naj bo $R = \{ c_1, c_2, \dots, c_r\}$ množica rangiranj množice $[n]$. Množici  vozlišč $C \subseteq [n]$ rečemo množica tekmovalcev, če vsaka dva elementa $i, j \in C$ tekmujeta in je $C$ maksimalna glede na to lastnost.
\end{definicija}
...
\begin{definicija}
    ...
    Množici vozlišč $D \subseteq [n]$ rečemo množica posrednih in neposrednih tekmovalcev, če vsaka dva elementa $i, j \in D$ posredno ali neposredno tekmujeta in je $D$ maksimalna glede na to lastnost.
\end{definicija}
...
\begin{dokaz}
    ...
    Naj bo $x \in D_{[1, p_{k+1}]}$, potem je $x \in [n]$ z $c_s^{-1}(x) \in [1, p_{k+1}]$ za nek $c_s$. Če je $c_s^{-1}(x) \in [1, p_k]$, sledi $x \in D_{[1, p_k]} \subseteq D$. V nasprotnem primeru je $c_s^{-1}(x) \in [p_k + 1, p_{k+1}]$. V tem primeru naj bo $b$ element $D_{[1,p_k]}$, ki se pojavi na poziciji $p_{k+1}$ v nekem rangiranju $c_{m_b}$, to pomeni $c_{m_b}^{-1}(b) = p_{k+1}$. Če $x \prec_{c_{m_b}} b$, potem je po lemi \ref{lema_algoritem_posredni_tekmovalci_1} element $x \in D$. V nasprotnem primeru je $b \prec_{c_{m_b}} x$. Vsi elementi levo od $b$ v rangiranju $c_{m_b}$ pripadajo množici $D$ po lemi \ref{lema_algoritem_posredni_tekmovalci_1}. Naj bo teh elementov $t$. Če je $x \prec_{c_s} b$, potem $(x, b)$ tekmujeta in $x \in D$. Zato predpostavimo, da $b \prec_{c_s} x$. Na levi od $x$ v rangiranju $c_s$ je tako največ $t$ elementov, ampak en od njih je $b$, kar pomeni, da obstaja element $z$, za katerega velja $z \prec_{c_{m_b}} b \prec_{c_{m_b}} x$ in $b \prec_{c_s} x \prec_{c_s} z$. To pomeni, da $(x, z)$ tekmujeta, zato $x \in D$.
...
\begin{dokaz}
    ...
    \begin{enumerate}
        ...
        \item Pokazali bomo, da, če je $a \in D_1$, $b \in D_2$ in obstaja usmerjena povezava od $a$ proti $b$, potem za vsak $b' \in D_2$ obstaja usmerjena povezava od $a$ do $b'$. Ker sta $b, b' \in D_2$, obstaja tak $k \in \mathbb{N}$ in $b_1, \dots, b_k \in D_2$, da $(b, b_1)$ tekmujeta, $(b_1, b_2)$ tekmujeta, $\dots$, $(b_k, b')$ tekmujeta. Vozlišča $a, b, b_1$ so v takem razmerju kot v koraku $1.$, zato obstaja usmerjena povezava od $a$ do $b_1$, podobno vozlišča $a, b_1, b_2$, zato obstaja usmerjena povezava od $a$ do $b_2$, $\dots$, podobno vozlišča $a, b_k, b'$, zato obstaja usmerjena povezava od $a$ do $b'$.
    \end{enumerate}
...
\label{definicija_relacije_mnozic_posrednih_tekmovalcev}
    Naj bo $R = \{ c_1, \dots, c_r \}$ množica $r$ rangiranj vozlišč $[n]$, $r \geq 2$, katerih množice posrednih in neposrednih tekmovalcev označimo z $D_1, \dots, D_k$, kjer je$\underset{i \in [k]}{\cup} D_i = [n]$. Definirajmo binarno relacijo $\rightarrow$ med dvema množicama posrednih in neposrednih tekmovalcev na naslednji način:
    \begin{enumerate}[label=(\roman*)]
...
Diplomsko delo smo začeli s ponovitvijo osnovnih pojmov, oznak in definicij s področja teorije grafov, algebre in teorije množic, ki so ključni za nadaljno razumevanje.
