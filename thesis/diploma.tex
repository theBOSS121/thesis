...
\label{def_tekmovalnosti}
    Če imamo podano končno množico $R = \{c_1, c_2, ..., c_r\}$ rangiranj, potem rečemo, da je par vozlišč $(i, j) \in [n] \times [n]$ (neposredno) tekmuje, če obstajata rangiranji $c_s, c_t \in R$ tako da je $i \prec_{c_s} j$ ampak $j \prec_{c_t} i$, to je i in j zamenjata svoji relativni poziciji v rangiranjih $c_s$ in $c_t$ (slika \ref{tekmovalnost_ij}).
...
\end{definicija}
Primer: Naj bo $R = \{ c_1, c_2, c_3 \}$ množica rangiranj množice [4].
\begin{align*}
    c_1 &= (1, 2, 3, 4) \notag\\
    c_2 &= (1, 2, 4, 3) \notag\\
    c_3 &= (3, 1, 2, 4) \notag
\end{align*}
Ker je je $c_1=id$ so povezave grafa $G_c(R)$ ravno inverzije rangiranj $c_2$ in $c_3$. Rangiranje $c_2$ ima inverzijo $(4, 3)$ medtem ko ima rangiranje $c_3$ inverziji $(3, 1), \ (3, 2)$. Graf $G_c(R)$ je prikazan na sliki \ref{graf_tekmovalnosti_primer}.
...
\begin{definicija}
    ...
    \]
\end{definicija}
Primer: Naj bo R tak kot v prejšnem primeru. Potem je:
\[
...
Primer: Naj bo $R = \{ c_1, c_2, c_3\}$ množica rangiranj $[5]$.
\begin{align}
    c_1 &= (1, 2, 3, 4, 5) \notag\\
    c_2 &= (2, 1, 3, 4, 5) \notag\\
    c_3 &= (1, 4, 2, 3, 5) \notag
...
\]
Elementa $1$ in $2$ se v rangiranjih nahajata na $1., 2.$ in $3.$ mestu. Ker $max\{ 1, 2, 3\} = 3$ si sedaj poglejmo $D_{[1, 3]}$
\[
    D_{[1,1]} \subset D_{[1,3]} = \{ x \in [n] \ | \ c_s^{-1}(x) \in [1, 3] \ za \ nek \ c_s \in R \} = \{ 1, 2, 3, 4\} \subseteq D_1
\]
Elementi $1, 2, 3$ in $4$ se v rangiranjih nahajajo na $1., 2., 3.$ in $4.$ mestu. Ker $max\{ 1, 2, 3, 4\} = 4$ si sedaj poglejmo $D_{[1, 4]}$
\[
...
\begin{definicija}
    Naj bo $R = \{ c_1, ..., c_r \}$ množica $r$ rangiranj $(r \geq 2)$ množice $[n]$. Definirajmo usmerjen graf $G_d(R)$  na naslenji način:
    \begin{enumerate}[label=(\roman*)]
...
\label{definicija_relacije_mnozic_posrednih_tekmovalcev}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item za vsaki različni množici $D_i, D_j$ posrednih in neposrednih tekmovalcev, je $D_i \rightarrow D_j$ naranko tedaj ko velja katerakoli od trditev iz \ref{trditev_ekvivalenc_mnozic_posrednih_tekmovalcev}
    \end{enumerate}
\end{definicija}
...
Poglejmo si sezono 2014 v prvenstvu MotoGP. V tej sezoni je bil najboljši dirkač Marc Márquez. Zmagal je na prvih desetih dirkah in trinajstih od skupaj osemnajstih dirk. Na 3 dirkah je padel, vendar se je v trenutku padca potegoval za zmago. Poleg tega je bil še enkrat drugi in enkrat četrti. Poleg Marka Marqueza so bili veliko boljši od ostalih še Valentino Rossi, Jorge Lorenzo in Dani Pedrosa. Na stopničkah so bili trije od njih na 13 dirkah, vsaj dva na 17 dirkah, vsaj en pa na vseh 18 dirkah te sezone. Tako opazimo, da je se nam nekako izoblikujejo vsaj tri skupine to sezono. V prvi skupini je Marc Márquez, ki je bil to sezono veliko boljši od ostalih. V drugi skupini so Valentino Rossi, Jorge Lorenzo in Dani Pedrosa. V ostalih skupinah pa so ostali dirkači.

Če bi uporabili algoritem za izračun množic posrednih in neposrednih tekmovalcev, kar narezultatih dirk, bi naleteli na težave. Ena od težav je, da ni na vsaki dirki tekmovalo oziroma zaključilo enako tekmovalcev. Zato rezultati dirk niso iz iste simetrične grupe $S_n$. Poleg tega opazimo, da je Marc Márquez bil na prvem in na zadnjem mestu (je odstopil), iz česar sledi, tekmuje z vsemi ostalimi dirkači in imamo tako samo eno množico posrednih in neposrednih tekmovalcev.
