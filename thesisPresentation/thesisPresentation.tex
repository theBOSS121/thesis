...
\frame{
    \frametitle{Permutacije}
    Permutacije so prerazporeditve elementov neke končne množice. Elemente te množice lahko oštevilčimo s števili $1, 2, \dots, n$, za nek $n$, zato bomo brez škode za splošnost permutacije gledali na množici $[n] = \{ 1, 2, \dots, n\}$. 
...
\frame{
    \frametitle{Zapis permutacije}
    Za zapis permutacije $\pi$ bomo uporabljali enovrstični zapis:
    \[
...
\frame{
    \frametitle{Inverzije permutacij}
    ...
    \end{block}
    Množico vseh inverzij permutacije $\sigma$ označimo z $I_{\sigma}$. Pozicijski zapis inverzije $(a_i, a_j)$ je $(i, j)$.

}
\frame{
    \frametitle{Število inverzij}    
    Število inverzij nam meri stopnjo neurejenosti permutacije oziroma oddaljenost permutacije od identične permutacije. 

    Število inverzij je enako številu presečišč v puščičnem diagramu permutacije. 
    % To je res, saj vsaka inverzija $(a_i, a_j)$ ustreza presečišču puščic, ki izhajajata iz $i$ in $j$ ter gresta proti $a_i$ in $a_j$, kjer je $a_i > a_j$ in $i < j$.
    Zato je število inverzij permutacije $\sigma$ enako številu inverzij permutacije $\sigma^{-1}$. 
...
\frame{
    \begin{figure}[h]
        \begin{center}        
            ...
            \end{tikzpicture}
        \end{center}
        \caption{ Gosenica $C_4(1, 1, 1, 1)$ je permutacijski graf permutacij $(2, 5, 1, 3, 6, 8, 4, 7)$ in $(3, 1, 4, 7, 2, 5, 8, 6)$. }
    ...
    \begin{figure}[h]
        \begin{center}        
            \begin{tikzpicture}[shorten >=1pt,-]
                ...
                \end{tikzpicture}     
        \end{center}
        \caption{Slika ponazarja odločitveno za permutaciji $(2, 4, 1, 6, 3, \dots)$ in $(3, 1, 5, 2, \dots)$, katerih permutacijski graf je $P_n$.}
...
\frame{
    \frametitle{Množice vozlišč v tekmovalnostenem grafu}
...
\frame{
    \frametitle{Algoritem za izrečun množice posrednih in neposrednih tekmovalcev}
